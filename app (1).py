...
def generar_ficha_completa(d):
    """Genera UN solo Excel con 4 hojas liberando memoria entre cada una."""
    import gc
    wb_final = Workbook()
    wb_final.remove(wb_final.active)

    buf1 = llenar_hoja1(d)
    wb1 = load_workbook(io.BytesIO(buf1.read()))
    copiar_hoja(wb1.active, wb_final, "EVALUACION OCUPACIONAL 1-3")
    wb1.close(); del wb1, buf1; gc.collect()

    buf2 = llenar_hoja2(d)
    wb2 = load_workbook(io.BytesIO(buf2.read()))
    copiar_hoja(wb2.active, wb_final, "EVALUACION OCUPACIONAL 2-3")
    wb2.close(); del wb2, buf2; gc.collect()

    buf3 = llenar_hoja3(d)
    wb3 = load_workbook(io.BytesIO(buf3.read()))
    copiar_hoja(wb3.active, wb_final, "EVALUACION OCUPACIONAL 3-3")
    wb3.close(); del wb3, buf3; gc.collect()

    bufc = llenar_certificado(d)
    wbc = load_workbook(io.BytesIO(bufc.read()))
    copiar_hoja(wbc.active, wb_final, "CERTIFICADO DE APTITUD")
    wbc.close(); del wbc, bufc; gc.collect()
