...
def borrador_guardar():
    ...
    try:
        ...
        try:
            existing = supa.table(TABLE_NAME).select("id").filter(
                "data->>N.° de historia clínica", "eq", cedula
            ).execute()
            if existing.data:
                row_id = existing.data[0]["id"]
                supa.table(TABLE_NAME).update({"data": fila}).eq("id", row_id).execute()
                print(f"✓ Borrador actualizado en matriz: {nombre} (cédula {cedula})")
            else:
                supa.table(TABLE_NAME).insert({"data": fila}).execute()
                print(f"✓ Borrador insertado en matriz: {nombre} (cédula {cedula})")
        except Exception as ex:
            # Si falla el upsert por cédula, insertar directo
            print(f"⚠ upsert error, insertando: {ex}")
            db_insertar(fila)
...
def importar_borradores():
    ...
    try:
        ...
        for b in (borradores.data or []):
            ...
            fila["N°"] = db_contar() + 1
            if existing.data:
                supa.table(TABLE_NAME).update({"data": fila}).eq("id", existing.data[0]["id"]).execute()
                omitidos += 1
            else:
                supa.table(TABLE_NAME).insert({"data": fila}).execute()
                importados += 1
