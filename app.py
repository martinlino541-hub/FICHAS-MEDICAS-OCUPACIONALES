...
def llenar_hoja2(d):
    ...
    # Medidas preventivas — A67:F71 label, G67:H68 celda dato
    med = d.get("medidasPreventivas","")
    sw(ws, "G67", med)
    if med:
        h = max(20, (len(med)//40+med.count("\n")+1)*14)
...
def llenar_certificado(d):
    ...
    # D. Recomendaciones — A24:AL26 -> maestra A24 (auto-height)
    reco = d.get("recomendaciones","")
    sw(ws, "A24", reco)
    if reco:
        h = max(30, (len(reco)//80+reco.count("\n")+1)*14)
        for r in [24, 25, 26]:
            ws.row_dimensions[r].height = h
