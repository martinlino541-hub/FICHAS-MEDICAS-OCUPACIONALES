...
def llenar_hoja2(d):
    ...
    # Medidas preventivas — A67:F71 label, G67:H68 celda dato (2 filas merged)
    med = d.get("medidasPreventivas","")
    sw(ws, "G67", med)
    if med:
        lineas = med.count("\n") + max(1, len(med)//40)
        # G67:H68 = 2 filas merged → altura por fila = total/2
        h_total = max(28, lineas * 14)
        h_fila  = h_total // 2
...
def llenar_certificado(d):
    ...
    # D. Recomendaciones — A24:AL26 (3 filas merged) → altura por fila = total/3
    reco = d.get("recomendaciones","")
    sw(ws, "A24", reco)
    if reco:
        lineas  = reco.count("\n") + max(1, len(reco)//80)
        h_total = max(42, lineas * 14)
        h_fila  = h_total // 3
        for r in [24, 25, 26]:
            ws.row_dimensions[r].height = h_fila
    else:
