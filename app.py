...
def generar_ficha_completa(d):
    """Genera UN solo Excel con 4 hojas: 3 de ficha + certificado."""
    wb_final = Workbook()
    # Eliminar hoja default
    wb_final.remove(wb_final.active)

    # ── Hoja 1 ──
    wb1 = load_workbook(io.BytesIO(llenar_hoja1(d).read()))
    copiar_hoja(wb1.active, wb_final, "EVALUACION OCUPACIONAL 1-3")

    # ── Hoja 2 ──
    wb2 = load_workbook(io.BytesIO(llenar_hoja2(d).read()))
    copiar_hoja(wb2.active, wb_final, "EVALUACION OCUPACIONAL 2-3")

    # ── Hoja 3 ──
    wb3 = load_workbook(io.BytesIO(llenar_hoja3(d).read()))
    copiar_hoja(wb3.active, wb_final, "EVALUACION OCUPACIONAL 3-3")

    # ── Certificado ──
    wbc = load_workbook(io.BytesIO(llenar_certificado(d).read()))
