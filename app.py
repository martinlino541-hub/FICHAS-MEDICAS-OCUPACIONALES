...
def llenar_hoja1(d):
    ...
    sw(ws, "A25", d.get("antClinico",""))
    sw(ws, "A27", d.get("antFamiliares",""))
    ...
    # F. Examen físico
    sw(ws, "A72", d.get("examenFisicoObs",""))
...
def combine_xlsx(sheets):
    """
    Combina múltiples xlsx en uno solo manipulando ZIP directamente.
    Usa ~10x menos memoria que copiar celda a celda.
    sheets = [(nombre_hoja, BytesIO), ...]
    """
    import zipfile, re, gc
    name1, buf1 = sheets[0]
    base_bytes = buf1.read(); del buf1; gc.collect()
    output = io.BytesIO()
    with zipfile.ZipFile(io.BytesIO(base_bytes), 'r') as base_z:
        with zipfile.ZipFile(output, 'w', zipfile.ZIP_DEFLATED) as out_z:
            ...
            for item in base_z.namelist():
                if item not in skip:
                    out_z.writestr(item, base_z.read(item))
            sheet_rels, sheet_entries = [], []
            for idx, (name, buf) in enumerate(sheets, 1):
                if idx == 1:
                    pass  # ya copiado arriba
                else:
                    with zipfile.ZipFile(io.BytesIO(buf.read()), 'r') as z:
                        out_z.writestr(f'xl/worksheets/sheet{idx}.xml', z.read('xl/worksheets/sheet1.xml'))
                    del buf; gc.collect()
                sheet_rels.append(f'<Relationship Id="rId{idx}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/worksheet" Target="worksheets/sheet{idx}.xml"/>')
                sheet_entries.append(f'<sheet name="{name}" sheetId="{idx}" r:id="rId{idx}"/>')
            # workbook.xml
            wb_xml = base_z.read('xl/workbook.xml').decode('utf-8')
            wb_xml = re.sub(r'<sheets>.*?</sheets>', f'<sheets>{"".join(sheet_entries)}</sheets>', wb_xml, flags=re.DOTALL)
            out_z.writestr('xl/workbook.xml', wb_xml)
            # workbook.xml.rels
            orig_rels = base_z.read('xl/_rels/workbook.xml.rels').decode('utf-8')
            orig_rels = re.sub(r'<Relationship[^/]*/>', '', orig_rels)
            orig_rels = orig_rels.replace('</Relationships>', ''.join(sheet_rels)+'</Relationships>')
            out_z.writestr('xl/_rels/workbook.xml.rels', orig_rels)
            # [Content_Types].xml
            ct_xml = base_z.read('[Content_Types].xml').decode('utf-8')
            for idx in range(2, len(sheets)+1):
                entry = f'<Override PartName="/xl/worksheets/sheet{idx}.xml" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.worksheet+xml"/>'
                if f'sheet{idx}.xml' not in ct_xml:
                    ct_xml = ct_xml.replace('</Types>', entry+'</Types>')
            out_z.writestr('[Content_Types].xml', ct_xml)
    del base_bytes; gc.collect()
    output.seek(0)
    return output


def generar_ficha_completa(d):
    """Genera UN solo Excel con 3 hojas usando combinación ZIP (eficiente en memoria)."""
    import gc
    b1 = llenar_hoja1(d); gc.collect()
    b2 = llenar_hoja2(d); gc.collect()
    b3 = llenar_hoja3(d); gc.collect()
    return combine_xlsx([
        ("EVALUACION OCUPACIONAL 1-3", b1),
        ("EVALUACION OCUPACIONAL 2-3", b2),
        ("EVALUACION OCUPACIONAL 3-3", b3),
    ])
