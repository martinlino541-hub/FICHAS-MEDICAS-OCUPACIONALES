...
def db_listar_con_id():
    ...
    if supa:
        try:
            # Intentar primero con select completo
            res = supa.table(TABLE_NAME).select("*").order("id").execute()
            result = []
            for r in res.data:
                # Supabase puede devolver {id:X, data:{...}} o el objeto plano
                if "data" in r and isinstance(r["data"], dict):
                    item = dict(r["data"])
                    item["_id"] = r.get("id", r.get("_id", 0))
                else:
                    # Si el row ES el data directamente
                    item = {k: v for k, v in r.items() if k != "id"}
                    item["_id"] = r.get("id", 0)
                result.append(item)
            print(f"✓ db_listar_con_id: {len(result)} registros desde Supabase")
            return result
        except Exception as e:
            print(f"⚠ db_listar_con_id error: {e}")
            # Fallback a db_listar normal
            try:
                filas = db_listar()
                return [dict(f, _id=i+1) for i, f in enumerate(filas)]
            except:
                pass
    # Fallback local
...
def api_registros():
    try:
        ...
        print(f"⚠ api_registros error: {e}")
        return jsonify([])


@app.route("/api/matriz/debug")
def api_debug():
    """Endpoint de diagnóstico — muestra estado de Supabase y conteo."""    info = {
    ...
    if supa:
        try:
            ...
            info["supabase_sample"] = res.data[:2] if res.data else []
        except Exception as e:
