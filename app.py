...
def ecnt_cargar():
    if not supa:
        return jsonify({"ok": False, "error": "Supabase no configurado"}), 503
    try:
        r = supa.table(ECNT_TABLE).select("cie10, nombre").order("cie10").execute()
        mapa = {row["cie10"]: row["nombre"] for row in (r.data or [])}
        return jsonify({"ok": True, "data": mapa})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
...
def ecnt_guardar():
    ...
    if not cie or not nombre:
        return jsonify({"ok": False, "error": "CIE-10 y nombre requeridos"}), 400
    try:
        supa.table(ECNT_TABLE).upsert(
            {"cie10": cie, "nombre": nombre}, on_conflict="cie10"
        ).execute()
        return jsonify({"ok": True})
    except Exception as e:
        return jsonify({"ok": False, "error": str(e)}), 500
