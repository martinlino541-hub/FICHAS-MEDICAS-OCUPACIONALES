...
def sw(ws, addr, value):
    """safe_write: escribe en celda maestra, aplica wrap_text y ajusta altura automáticamente."""
    ...
    def _write(cell, val):
        ...
        if val and isinstance(val, str):
            ...
            # Auto-ajuste de altura para texto largo
            row = cell.row
            cols_aprox = 80  # caracteres por línea aproximado
            lineas = val.count('\n') + max(1, len(val) // cols_aprox)
            if lineas > 1:
                current = ws.row_dimensions[row].height or 15
                ws.row_dimensions[row].height = max(current, min(lineas * 14, 200))

    try:
        _write(ws[addr], value)
    except AttributeError:
        # Celda secundaria de rango combinado — buscar celda maestra
        try:
...
def llenar_hoja1(d):
    ...
    # Reproductivos masculinos — A39:M40=label, N39:S40=TIEMPO label, T39:AC40=nota
    sw(ws, "A39",  d.get("examRepMasc",""))
    sw(ws, "N39",  d.get("tiempoRepMasc",""))
    sw(ws, "AE40", d.get("metPlanMasc",""))
    ...
    # Estilo de vida — AH45=checkbox actFisica, AK45=tiempo, AT45=cuál (fila 45 datos)
    if d.get("actFisica"): sw(ws, "AH45", "X")
    sw(ws, "AK45", d.get("actFisicaT",""))
    sw(ws, "AT45", d.get("actFisicaCual",""))

    # Medicación habitual — AT45=cuál, BB45=cantidad (fila 45 datos)
...
def llenar_certificado(d):
    ...
    # E. Datos profesional — A33=nombre (sobrescribe label, como Diana), L33=código
