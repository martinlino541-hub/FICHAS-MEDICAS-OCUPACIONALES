...
def llenar_hoja1(d):
    ...
    # Grupo atención prioritaria — fila 13 (vacía, debajo de los labels 10-12)
    if d.get("embarazada"):   sw(ws, "A13", "X")
    if d.get("discapacidad"): sw(ws, "F13", "X")
    if d.get("enfCatastrofica"): sw(ws, "J13", "X")
    if d.get("adultoMayor"):  sw(ws, "M13", "X")
    sexo = d.get("sexo","")
...
def llenar_certificado(d):
    ...
    # E. Datos profesional — E33=nombre (A33:D33 es label), O33=código (L33:N33 es label)
