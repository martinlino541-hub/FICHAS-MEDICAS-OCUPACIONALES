...
def llenar_hoja1(d):
    ...
    if regiones:
        ...
        for region, data in regiones.items():
            if isinstance(data, dict):
                if data.get("checked"):
                    ...
                    if hallazgo:
                        con_patologia.append(f"{region}: {hallazgo}")
                    else:
                        con_patologia.append(f"{region}: PATOLOGÍA (sin descripción)")
                else:
                    # Extraer solo el número de región para la lista de normales
        ...
        if normales:
            lineas.append("NORMAL: regiones " + ", ".join(normales))
        if obs_adicional:
            lineas.append("Observaciones adicionales: " + obs_adicional)
    ...
    if texto_examen:
        chars_por_linea = 120
        lineas_count = max(3, len(texto_examen) // chars_por_linea + texto_examen.count('\n') + 2)
        altura = max(45, lineas_count * 15)
...
def print_setup(ws, orientacion="portrait"):
    """Configura impresión A4, ajuste a una página, márgenes mínimos."""
    from openpyxl.worksheet.page import PageMargins
    ws.page_setup.paperSize   = ws.PAPERSIZE_A4      # A4
    ws.page_setup.orientation = orientacion           # portrait / landscape
    ws.page_setup.fitToPage   = True
    ws.page_setup.fitToWidth  = 1                    # 1 página de ancho
    ws.page_setup.fitToHeight = 1                    # 1 página de alto
