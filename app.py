...
def db_listar():
    if supa:
        try:
            ...
            return [r["data"] for r in res.data] + _matriz_local
        except Exception as e:
...
# ── BORRADORES ────────────────────────────────────────
def borrador_guardar_db(cedula, data):
    if supa:
        try:
            supa.table("borradores_femo").upsert(
                {"cedula": cedula, "data": data}, on_conflict="cedula"
            ).execute()
            return True
        except Exception as e:
            print(f"⚠ Borrador save error: {e}")
    return False

def borrador_cargar_db(cedula):
    if supa:
        try:
            ...
            print(f"⚠ Borrador load error: {e}")
    return None

def borrador_eliminar_db(cedula):
    if supa:
        try:
            supa.table("borradores_femo").delete().eq("cedula", cedula).execute()
        except Exception as e:
            print(f"⚠ Borrador delete error: {e}")

# ─────────────────────────────────────────────────────
def sw(ws, addr, value):
    """safe_write: escribe en celda maestra de rangos combinados + wrap_text."""
...
def llenar_hoja1(d):
    ...
    if texto_examen:
        lc = max(3, len(texto_examen) // 120 + texto_examen.count('\n') + 2)
        h  = max(45, lc * 15)
        ws.row_dimensions[72].height = h
        ws.row_dimensions[73].height = h
...
# ─────────────────────────────────────────────────────
#  COPIAR HOJA (con colores RGB)
# ─────────────────────────────────────────────────────
INDEXED_TO_RGB = {
    9:  'FFFFFFFF', 27: 'FFCCFFFF', 31: 'FFCCCCFF', 42: 'FFCCFFCC',
...
def get_fill_rgb(fill):
    if fill is None or fill.fill_type not in ('solid', 'patternFill'):
        return None
    fg = fill.fgColor
    if fg is None:
        return None
    if fg.type == 'rgb' and fg.rgb not in ('00000000', 'FF000000', '000000FF'):
        return fg.rgb
    if fg.type == 'indexed':
...
def copiar_hoja(ws_origen, wb_destino, nombre_hoja):
    ...
    # Copiar rangos combinados
    for mr in ws_origen.merged_cells.ranges:
...
def generar_ficha_completa(d):
    """Genera Excel con 3 hojas de evaluación."""
    import gc
    wb_final = Workbook()
...
def api_ficha():
    ...
    return send_file(buf, mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                     as_attachment=True, download_name=f"FichaOcupacional_{nombre}_{fecha}.xlsx")
...
def api_cert():
    try:
        ...
        return send_file(buf, mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                         as_attachment=True, download_name=f"Certificado_Aptitud_{nombre}_{fecha}.xlsx")
    except Exception as e:
        import traceback
        print("ERROR certificado:", traceback.format_exc())
...
def api_agregar():
    ...
    try:
        ...
        _matriz_local.append(fila)
        return jsonify({"ok": True, "total": len(_matriz_local), "nombre": nombre})
...
def borrador_guardar():
    ...
    try:
        supa.table("borradores_femo").upsert(
            {"cedula": cedula, "data": d}, on_conflict="cedula"
        ).execute()
