...
# Convertir XLS a XLSX automáticamente si no existe la versión XLSX
def _convertir_xls_a_xlsx():
    xls = os.path.join(PLANTILLAS, "Formulario_evaluaciones_ocupacionales.xls")
    xlsx = os.path.join(PLANTILLAS, "Formulario_evaluaciones_ocupacionales.xlsx")
    if os.path.exists(xls) and not os.path.exists(xlsx):
        try:
            import subprocess
            subprocess.run(
                ["libreoffice","--headless","--convert-to","xlsx",xls,"--outdir",PLANTILLAS],
                capture_output=True, timeout=60
            )
            print("✓ XLS convertido a XLSX")
        except Exception as e:
            print(f"⚠ No se pudo convertir XLS: {e}")

_convertir_xls_a_xlsx()
...
def llenar_hoja1(d):
    ...
    # Convertir XLS → XLSX en memoria via libreoffice (ya convertido)
    wb = lw(os.path.join(PLANTILLAS, "Formulario_evaluaciones_ocupacionales.xlsx"))
