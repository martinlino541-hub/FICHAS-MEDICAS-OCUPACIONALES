...
def api_estadisticas2():
    ...
    for f in filas:
        for c1, c2 in [("DIAGNÓSTICO 1 CIE-10","DESCRIPCIÓN DX1"),("DIAGNÓSTICO 2 CIE-10","DESCRIPCIÓN DX2")]:
            ...
            # Z00.x = examen sin hallazgos → mostrar como "Paciente sano" solo si todos son Z00
    ...
    pacientes_sanos = 0
    for f in filas:
        cie1 = str(f.get("DIAGNÓSTICO 1 CIE-10","") or "").strip().upper()
        cie2 = str(f.get("DIAGNÓSTICO 2 CIE-10","") or "").strip().upper()
        # Solo Z00 o sin diagnóstico = paciente sano / examen normal
        dx_validos = [c for c in [cie1, cie2] if c and not c.startswith("Z00")]
        if not dx_validos:
            pacientes_sanos += 1
