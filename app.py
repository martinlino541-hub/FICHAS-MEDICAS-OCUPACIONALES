...
def sw(ws, addr, value):
    """safe_write: escribe en la celda maestra de rangos combinados."""
    from openpyxl.utils import coordinate_to_tuple
    try:
        ws[addr] = value
    except AttributeError:
        # La celda es secundaria de un rango combinado — buscar la maestra
        try:
...
def llenar_hoja1(d):
    ...
    # F. Examen físico — procesar sub-ítems por sección
    ef_items   = d.get("examenFisicoItems", {})
    ef_regiones = d.get("examenFisicoRegiones", {})
    ...
    lineas = []
    if ef_items:
        for (num, nombre, items) in EF_SECCIONES:
            hallazgos_sec = []
            normales_sec = []
            for ii, item in enumerate(items):
                key = f"{num}|{ii}"
                dato = ef_items.get(key, {})
                if isinstance(dato, dict) and dato.get("checked"):
                    desc = dato.get("desc","").strip()
                    hallazgos_sec.append(f"{chr(96+ii+1)}. {item}" + (f": {desc}" if desc else ""))
                else:
                    normales_sec.append(chr(96+ii+1))
            if hallazgos_sec:
                lineas.append(f"{num}. {nombre} — HALLAZGO: {'; '.join(hallazgos_sec)}")
            else:
                lineas.append(f"{num}. {nombre}: Normal")
    elif ef_regiones:
        for region, data in ef_regiones.items():
            if isinstance(data, dict):
                if data.get("checked"):
                    h = data.get("hallazgo","").strip()
                    lineas.append(f"{region}: PATOLOGÍA" + (f" — {h}" if h else ""))
                else:
                    lineas.append(f"{region}: Normal")

    if obs_adicional:
        lineas.append(f"Observaciones adicionales: {obs_adicional}")

    texto_examen = "; ".join(lineas) if lineas else "Examen físico regional sin hallazgos patológicos."

    # Escribir en A72 (o celda maestra de ese rango)
    sw(ws, "A72", texto_examen)
    if texto_examen:
        chars_por_linea = 100
        lineas_count = max(4, len(texto_examen) // chars_por_linea + texto_examen.count('\n') + 2)
        altura = max(60, lineas_count * 14)
...
def llenar_hoja3(d):
    ...
    for i, dx in enumerate(diagnosticos[:6]):
        if i < len(filas_dx):
            ...
            tipo = dx.get("tipo","PRE")
            sw(ws, f"BE{f}", "X" if tipo == "PRE" else "")
            sw(ws, f"BJ{f}", "X" if tipo == "DEF" else "")
...
def llenar_certificado(d):
    ...
    # D. Recomendaciones
    # Limpiar celda maestra antes de escribir recomendaciones
    try:
        ws["A24"] = ""
    except AttributeError:
        for mr in ws.merged_cells.ranges:
            if mr.min_row == 24 and mr.min_col == 1:
                ws.cell(mr.min_row, mr.min_col).value = ""
                break
...
def generar_ficha_completa(d):
    """Genera UN solo Excel con 4 hojas — ficha + certificado."""
    import gc
    wb_final = Workbook()
    ...
    # 4ta hoja: Certificado de Aptitud
    buf_cert = llenar_certificado(d)
    wb_cert = load_workbook(io.BytesIO(buf_cert.read()))
    copiar_hoja(wb_cert.active, wb_final, "CERTIFICADO")
    wb_cert.close()
    del wb_cert, buf_cert
    gc.collect()
...
def api_historia():
    ...
    return send_file(
        ...
        as_attachment=True,
        download_name=f"HistoriaClinica_{nombre}_{fecha}.xlsx"
    )
