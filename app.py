...
def generar_ficha_completa(d):
    ...
    HOJAS = [
        (llenar_hoja1, "EVALUACION OCUPACIONAL 1-3",  '-EVALUACION OCUPACIONAL 1-3'),
        (llenar_hoja2, "EVALUACION OCUPACIONAL 2-3",  'EVALUACION OCUPACIONAL2-3'),
        (llenar_hoja3, "EVALUACION OCUPACIONAL 3-3",  'EVALUACION OCUPACIONAL 3-3'),
    ]

    for fn, nombre_destino, nombre_origen in HOJAS:
        buf = fn(d)
        wb_src = load_workbook(io.BytesIO(buf.read()))
        # Tomar la hoja correcta por nombre, no la activa
        ws_origen = wb_src[nombre_origen] if nombre_origen in wb_src.sheetnames else wb_src.active
        copiar_hoja(ws_origen, wb_final, nombre_destino)
        wb_src.close()
        del wb_src, buf
        gc.collect()
