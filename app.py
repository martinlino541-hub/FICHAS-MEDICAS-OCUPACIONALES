...
def db_listar_con_id():
    """
    Usa db_listar() (que ya funciona) y agrega _id desde el campo N°
    o desde el id de Supabase en una segunda consulta.
    """
    # Primero obtener todos los datos (esto YA funciona)
    filas = db_listar()
    if not filas:
        return []

    # Intentar obtener los IDs reales de Supabase para poder editar/eliminar
    ids_map = {}
    if supa:
        try:
            ...
            for r in res.data:
                try:
                    numero = str(r["data"].get("N°", ""))
                    nombre = str(r["data"].get("NOMBRE COMPLETO", ""))
                    key = f"{numero}_{nombre}"
                    ids_map[key] = r["id"]
                except:
                    pass
        except Exception as e:
            print(f"⚠ ids_map error: {e}")

    result = []
    for i, fila in enumerate(filas):
        item = dict(fila)
        # Buscar ID real de Supabase
        numero = str(item.get("N°", ""))
        nombre = str(item.get("NOMBRE COMPLETO", ""))
        key = f"{numero}_{nombre}"
        if key in ids_map:
            item["_id"] = ids_map[key]
        else:
            item["_id"] = i + 1  # fallback: índice
        result.append(item)
    return result
