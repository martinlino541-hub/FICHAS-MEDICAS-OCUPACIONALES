...
def api_agregar():
    ...
    fila = {
        ...
        # Guardar hasta 6 diagnósticos
        **{f"DIAGNÓSTICO {i+1} CIE-10": (dx.get("cie10","") or "")
           for i, dx in enumerate((d.get("diagnosticos") or [])[:6])
           if (dx.get("cie10","") or "").strip()},
        **{f"DESCRIPCIÓN DX{i+1}": (dx.get("desc","") or "")
           for i, dx in enumerate((d.get("diagnosticos") or [])[:6])
           if (dx.get("cie10","") or "").strip()},
        # Compatibilidad: alias para Dx1 y Dx2 (usados en estadísticas)
        "DIAGNÓSTICO 1 CIE-10": (d.get("diagnosticos",[{}])[0] or {}).get("cie10","") if d.get("diagnosticos") else "",
        "DESCRIPCIÓN DX1": (d.get("diagnosticos",[{}])[0] or {}).get("desc","") if d.get("diagnosticos") else "",
